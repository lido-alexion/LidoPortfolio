...
The remaining direct Telegram inventory is explicit: `NotificationEngine` still owns the legacy `TosNotification` history/compatibility contract used by recommendation, execution and lending flows, while `AlertNotificationService` retains only the legacy opt-in clear-ping and manual credential test paths. These paths require dedicated compatibility tests before migration because replacing their delivery rows or history semantics in place could regress shipped history or retry behavior.
