...
The remaining direct Telegram inventory is explicit: `NotificationEngine` still owns the legacy `TosNotification` history/compatibility contract used by recommendation, execution and lending flows; `AlertNotificationService` retains only the legacy opt-in clear-ping and manual credential test paths; `AdminOperationalAlertService` still owns admin-wide operational fan-out; and stock-price sync services retain failure-alert hooks. These paths require dedicated compatibility tests before migration because replacing their delivery rows or admin recipient semantics in place could regress shipped history, retry or operational-alert behavior.
