...
The remaining direct Telegram inventory is limited to `AlertNotificationService`'s legacy opt-in clear-ping and manual credential test paths. These require a deliberate product decision because a Portfolio-scoped credential test must not silently test a different account-level destination, while the global clear-ping flag does not yet have an account-level preference equivalent.
