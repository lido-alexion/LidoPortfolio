...
Current count: **0 OPEN / 2 IN PROGRESS / 8 DECIDED / 7 COMPLETE / 1 SUPERSEDED**.
...
| V4-FEAT-004 | Notification channel abstraction + email/webhook | **Implementation in progress:** [`V5-FEAT-004-Notification-Service.md`](V5-FEAT-004-Notification-Service.md). Canonical lifecycle storage, account-scoped APIs, both-role in-app UX, encrypted account-level settings and real Telegram/Email/signed-Webhook verification tests now implement strict audience isolation, attention/condition separation, filtered history, read actions, active deduplication, escalation, recurrence and verification-before-enable gates. Remaining work includes production delivery adapters/jobs, retry/reminders, health conditions, settings UI, optional email verification and migration of existing producers. FEAT-004 absorbs FEAT-003. | P2 | IN PROGRESS |
