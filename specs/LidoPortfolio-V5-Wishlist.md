...
| V4-FEAT-004 | Notification channel abstraction + email/webhook | **Implementation in progress:** [`V5-FEAT-004-Notification-Service.md`](V5-FEAT-004-Notification-Service.md). The canonical source, per-account recipient projection and immutable occurrence-history foundation now implements strict audience fan-out, independent attention/condition state, active-condition deduplication, severity escalation and resolved-then-recurrent lifecycle. Remaining work includes account-level APIs/UI, channel settings/adapters, durable delivery/retry/reminders, health conditions and migration of existing producers. FEAT-004 absorbs FEAT-003. | P2 | IN PROGRESS |
