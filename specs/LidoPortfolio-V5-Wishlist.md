...
| V4-FEAT-004 | Notification channel abstraction + email/webhook | **Implementation in progress:** [`V5-FEAT-004-Notification-Service.md`](V5-FEAT-004-Notification-Service.md). The canonical source, per-account recipient projection, immutable occurrence history and account-scoped Notification Center API now implement strict audience isolation, independent attention/condition state, filtered/paginated history, unread/Critical counts, mark-read/all-read, active-condition deduplication, severity escalation and resolved-then-recurrent lifecycle. Remaining work includes frontend Center/bell/banner, channel settings/adapters, durable delivery/retry/reminders, health conditions and migration of existing producers. FEAT-004 absorbs FEAT-003. | P2 | IN PROGRESS |
