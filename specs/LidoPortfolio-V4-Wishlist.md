...
| V4-FEAT-037 | Dashboard-first daily Kite readiness and reconnect | V5 broker-operations UX: when an Automatic portfolio cannot submit because the daily Kite session is missing/expired, show a prominent Dashboard readiness state with a minimum-friction **Connect Kite** action; after Zerodha authentication, return to Dashboard and refresh readiness. Add a configurable Telegram reminder time (existing app timezone) that sends at most once per day only while Automatic execution is enabled and Kite remains unusable; suppress it when already connected. This streamlines Zerodha's mandatory interactive daily login—it does **not** claim silent token renewal. | P2 | OPEN |
