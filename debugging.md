...
3. **If no runs tonight** — `cpanel-read-logs.php?file=scheduler&grep=heartbeat` — heartbeats every minute?
   ...
   - **`would_skip_reason=weekend_skip`** → Sat/Sun and the prior session’s **last** maintenance batch succeeded (expected; set `UNIVERSE_MAINTENANCE_SKIP_WEEKENDS=false` only if you want every weekend). Ops **universe/daily overdue** Telegram is also suppressed in that case.
   - **Daily/benchmark/index sync on Sat/Sun** → scheduler skips non-session days; manual `portfolio:daily-sync` still works. Daily overdue ops alerts stay silent if Friday’s daily sync succeeded.
