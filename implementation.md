...
**V5 FEAT-037 Dashboard Kite readiness — first slice (2026-09-04):** Automatic portfolios now check the existing broker status independently of cached Dashboard analytics and show a prominent reconnect warning only when Kite is unusable. Dashboard-initiated login stores an allowlisted return target inside the existing encrypted short-lived state, so successful authentication returns to Dashboard while Account settings retains its current return. The at-most-once-daily configurable Telegram reminder remains to complete FEAT-037.
