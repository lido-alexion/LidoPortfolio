...
- Tabular UIs: **TanStack Table** via `DataTableCard` / `DataTable.jsx` (columns icon in card header; show/hide + reorder panel; **drag column edges to resize** — widths persisted in `localStorage` with order/visibility via `tableColumnPrefs.js`, reset via **Reset columns**). **Fit columns** toolbar button (expand icon beside columns menu) redistributes visible column widths proportionally to fill the table container (`distributeColumnWidths` in `tableColumnPrefs.js`) — removes empty right-edge gap or horizontal scrollbar until the user resizes again. Pass `loading` while fetching — shows a spinner row instead of the empty-state message until data arrives (`TableLoadingRow`). Used on holdings, transactions, closed transactions, dashboard tables (alerts, relative strength, allocation, pattern signals), watchlist scan results, stock prices OHLCV, and corporate-action preview. Plain HTML tables on Settings and sync logs are not resizable.
