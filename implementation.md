...
**Architecture specs reorganization (2026-08-06):** Documentation-only move into domain folders under `specs/architecture/` (`platform/`, `ui/`, `indicators/`, `portfolio/`, `data/`, `engines/`, `live-trading/`, `governance/`, `audit/`). Filenames unchanged. Summary: [`specs/architecture/MIGRATION-SUMMARY.md`](specs/architecture/MIGRATION-SUMMARY.md).
...
- **API (admin):** `GET /api/v1/indicators`, `/meta`, `/{id}` — see `specs/architecture/engines/Indicator-Registry-API.md`.
...
**Weekend skip + Friday-failure retry (Aug 2026):** Markets are closed Sat/Sun, so scheduled universe maintenance **skips weekends by default** (`UNIVERSE_MAINTENANCE_SKIP_WEEKENDS=true`). If the **prior equity session** (typically Friday) had any **failed/partial** `universe-price-sync` batch during that night’s maintenance window, weekend slots **re-open** to heal (`UNIVERSE_MAINTENANCE_WEEKEND_RETRY_ON_FAILURES=true`). Manual UI/CLI runs are unchanged. Ops “maintenance overdue” (45‑min threshold) only applies when the calendar day is actually allowed to run.
