...
**API:** `GET/POST /api/screeners`, `GET /api/screeners/shared`, `POST /api/screeners/shared/{id}/import`, `GET/PUT/DELETE /api/screeners/{id}`, `GET /api/screeners/meta`, `POST /api/screeners/{id}/run`, `GET /api/screeners/{id}/runs`, `GET /api/screeners/{id}/runs/compare` (stock×run presence matrix for stacked UI), `DELETE /api/screeners/{id}/runs` (clear all run history + hits), `POST /api/screeners/{id}/backtest` (body `range` + `session_token`; holdings/watchlist only), `POST /api/screener-backtests/{id}/continue`, `GET /api/screener-backtests/{id}/matrix`, `DELETE /api/screener-backtests/session/{token}`, `GET /api/screener-runs/{run}`, `POST /api/screener-runs/{run}/continue`.
...
**Backtest (Jul 2026):** Editor footer **Backtest** split button (`ComboButton`, same pattern as Add to watchlist) — primary runs the selected window; dropdown picks `1y` / `6m` / `3m` / `1m` / `15d` (holdings/watchlist only). Walks weekdays from window start → today (Sat/Sun skipped); each day evaluates with OHLCV `price_date <= as_of`. Session-scoped tables `portfolio_screener_backtests` + `portfolio_screener_backtest_hits` (`session_token`); chunked by `BACKTEST_DAY_CHUNK` days via continue endpoint; matrix reuses stacked compare UI. Discard on editor unmount / new backtest / `DELETE .../session/{token}`.
...
**Tests:** `tests/Unit/Screener/TechnicalIndicatorServiceTest.php`, `tests/Unit/Screener/ScreenerBacktestCalendarTest.php`, `tests/Feature/ScreenerTest.php`.
