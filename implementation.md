...
| Recommendation | `Recommendation\RecommendationEngine` | Lifecycle only (Approve/Reject/Defer; pending-execution / cancel-execution / expire / reopen; cash reservation); generation delegated to `Recommendation\RecommendationGenerationPipeline` (TD-002) — Strategy scoring → Market Opinion → Portfolio Decision → Ranking → Capital Allocation → Trade gen |
