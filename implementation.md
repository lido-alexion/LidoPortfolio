...
- To share a single MySQL database with an existing project, this app uses isolated table names:
  ...
  - `portfolio_alerts`, `portfolio_settings`, `portfolio_user_settings`, `portfolio_system_logs`
...
- **Telegram notifications** are separate from **data syncing time** (`cron_time`). Per-user `notification_schedules` (JSON in `portfolio_user_settings`) drive `portfolio:send-notifications` — Laravel scheduler registers the **union** of all users' times; each run sends only to users whose schedule includes that slot. `AlertNotificationService` sends **that user's** active alerts to **that user's** Telegram credentials. Stoploss triggers only **persist** alerts (no immediate Telegram). Settings UI: add/remove notification times; **Test telegram integration** sends **only the logged-in user's** alerts.
- **Per-user settings (Jun 2026):** `portfolio_user_settings` (`user_id` + `setting_key`) stores per account: `telegram_bot_token`, `telegram_chat_id`, `notifications_enabled`, `default_stoploss_percent`, `notification_schedules`. Migration copies prior global `portfolio_settings` values to every existing user, then removes those keys from global settings. `GET/PUT /api/settings` merges global app settings (cron, fees, API keys, log level) with the authenticated user's personal settings. **Trailing stop %** in holdings/alerts uses the logged-in user's `default_stoploss_percent`. **No admin/user roles** — `portfolio_users` has no `role` column; seeded user is named "Portfolio Admin" but has the same permissions as any other account (any logged-in user can still edit global settings such as cron time and fee components).
...
- **Alert expiration** (`portfolio_alerts.user_id`, `expired_at`, `expiration_reason`): alerts are **per user** (not global per stock). Stoploss creates one alert per holder per day (`user_id` + `stock_id` dedup). Message includes latest close (with price date), trailing stop, stop %, highest close since buy, and peak date (per-user via `HoldingPresentationService`). **Trigger** uses the same since-buy `latest_close` vs `trailing_stop_price` from `HoldingPresentationService` (not all-time historical peak — fixes false alerts when pre-buy highs implied a tighter global trailing stop). `firstBuyDateForCurrentPosition` resets after a full exit, so a re-buy tracks peak/trailing only from the new position (not the prior round-trip). `portfolio_stock_metrics.highest_close` / `trailing_stop_price` use max since-buy peak across active holders and do not fall back to stale pre-exit metric values. Dashboard alerts table shows **Date** (`created_at`). `GET /api/alerts` / dashboard filter by `user_id`. Expiration: manual clear all + acknowledge (own alerts only); hourly 100h max age; new trading day after daily sync; **full sell** expires only that user's alerts (`expireForUserStockIfUnheld`). Active = `expired_at` IS NULL. Stale stoploss alerts are not auto-cleared when price recovers above the trailing stop.
- Dashboard summary cards (Portfolio Value, Invested, Gain/Loss, XIRR, Top Gainer/Loser): responsive grid — **3** per row (`col-lg-4`) on wide viewports, **2** (`col-md-6`) on medium, **1** (`col-12`) on narrow. Allocation **Market Value**, growth-chart axis/tooltips use `formatInrWhole` / `formatInrCompactWhole` (no paise; `₹ ` + amount, lakh grouping). Holdings/Explorer use `formatInr` (2 dp) via `formatTableMoney2`.
...
- Tabular UIs: **TanStack Table** via `DataTableCard` / `DataTable.jsx` (columns icon in card header; show/hide + reorder panel). Pass `loading` while fetching — shows a spinner row instead of the empty-state message until data arrives (`TableLoadingRow`). Used on holdings, transactions, closed transactions, stock prices, and sync logs tables.
...
- Holdings table **Sell** button navigates to `/transactions` with form prefilled: symbol/name/exchange, type `sell`, quantity = holding qty, price = latest close, symbol marked validated (`sellTransactionPrefill.js` + router `location.state`). **Invested** column: list icon link (hidden until row/cell hover; tooltip **View transactions**) → `/transactions` with `transactionSearch` set to the stock symbol.
...
- Holdings table default column order: Stock → Latest Close → Invested → Fees → XIRR → Highest Close → **Qty** → **Avg Buy** → Trailing Stop → Realized P/L → OHLCV → Sell. **Fees** and **Realized P/L** hidden by default (`defaultColumnVisibility`); column prefs stored per view in `localStorage` keys `portfolio_datatable_holdings` (complex) and `portfolio_datatable_holdings-simple` (simple). **Simple / Complex** toggle in card header (before columns menu): complex = current multi-line cells; simple = primary value only (no since-buy date, price date, LTP gain/loss % in parentheses, fee %, LTP drawdown, stop %, OHLCV row count, etc.). Selected view persisted in `portfolio_holdings_view`. Two table instances share the same dataset; only the active view is shown; column menu applies to the visible view only.
