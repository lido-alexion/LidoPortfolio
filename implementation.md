...
- **Multi-portfolio UI (Jun 2026):** Header switcher shown only when user has 2+ portfolios (10px left margin); profile menu links to `/portfolios`. Settings removed from top tabs (footer nav only). Settings page scope tabs: **Global** (admin), **Portfolio**, **Account** (sessions + prominent management links). Portfolio names: letters, numbers, spaces, hyphen, underscore only (client + API validation).
