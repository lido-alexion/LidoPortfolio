...
- **TD-002 (2026-07-27, code audit remediation):** Extracted `RecommendationEngine::generate()` (previously one large method + generation-only helpers) into `Recommendation\RecommendationGenerationPipeline`. Pipeline owns staged orchestration — `prepareContext` (eval run, strategy/config, thresholds, market gates/multipliers, cash headroom, eligibility, holdings, portfolio valuation) → `cancelStaleRecommendations` → `buildDrafts` (per-candidate scoring/opinion/decision/exit-eval/plan) → `rankDrafts` (tie-break sort) → `allocateCapital` (buy-draft shaping + `CapitalAllocationStrategy::allocate`) → `persistDrafts` (apply allocation, build evidence/reasoning, create `TradingRecommendation` rows) — plus the generation-only helpers (`buildMarketOpinion`, `decidePortfolioAction`, `buildExecutionPlan`, `wholeShareQuantity`, `riskExplanation`, `buildReasoning`, `latestClose`, `riskLevel`). No behaviour change; verified via `TradingOsPipelineTest`, `StrategyConfigurationServiceTest`, `MarketAnalysisEngineTest` (13 tests / 85 assertions, all passing).
- **TD-001 (2026-07-27, code audit remediation):** Split lifecycle out of `RecommendationEngine` into new `Recommendation\RecommendationLifecycleService`, moved **verbatim** (same signatures/behaviour): `recordReview`, `reserveForApproval`, `releaseReservation`, `convertReservation`, `cancelExecution`, `markExpired`, `reopenForReview`, `expireStale`, `listForProfile`, `listOpenForReview`, `listPendingExecution`, `listActive`, `findForProfile`, `history`, `reviewHistory`. New service's constructor needs only `PortfolioLoggerService` + `CashManagementService` (the only deps those methods used). `RecommendationEngine` is now a pure façade: constructor slimmed to `RecommendationGenerationPipeline` + `RecommendationLifecycleService` only (dropped now-unused `portfolio`/`strategies`/`eligibility`/`marketAnalytics`/`allocator` deps — those live on the pipeline, which Laravel still auto-builds since it's a plain concrete-dependency class); every public method is a one-line delegate to the pipeline or lifecycle service, so `TradingOsController`, `ExecutionEngine`, `DailyDecisionPipeline` (all type-hinted to `RecommendationEngine`) needed **zero changes**. No behaviour change; verified via `TradingOsPipelineTest` (4 tests / 60 assertions, passing) plus full suite (400/405 passing — 5 pre-existing unrelated failures: cash-balance validation in `TransactionStockResolverTest`, `StockPriceHistoryServiceTest` growth calc, `ExplorerAnalyticsTest` benchmark value, `DailyMarketDataJobTest` `db.schema` container binding — none touch Recommendation code).
