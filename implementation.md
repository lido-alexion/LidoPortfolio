...
### Pending deploy (2026-06-29 — multi-portfolio)
...
- **Multi-portfolio UI (Jun 2026):** Header switcher shown only when user has 2+ portfolios (10px left margin); profile menu links to `/portfolios`. Settings removed from top tabs (footer nav only). Settings page scope tabs: **Global** (admin), **Portfolio**, **Account** (sessions + prominent management links). Portfolio names: letters, numbers, spaces, hyphen, underscore only (client + API validation). **Manage Portfolios** (`/portfolios`) uses theme-aware `.contentPane .card` / `.list-group-item` (no hardcoded `bg-dark`); **Set default** uses `btn-outline-primary` so label is visible in light and dark themes.
...
- **Alert expiration** (`portfolio_alerts.profile_id`, `expired_at`, `expiration_reason`): alerts are **per profile** (not global per stock). Stoploss creates one alert per profile holder per day (`profile_id` + `stock_id` dedup). Message includes latest close (with price date), trailing stop, stop %, highest close since buy, and peak date (per-profile via `HoldingPresentationService`). **Trigger** uses the same since-buy `latest_close` vs `trailing_stop_price` from `HoldingPresentationService` (not all-time historical peak — fixes false alerts when pre-buy highs implied a tighter global trailing stop). `firstBuyDateForCurrentPosition` resets after a full exit, so a re-buy tracks peak/trailing only from the new position (not the prior round-trip). `portfolio_stock_metrics.highest_close` / `trailing_stop_price` use max since-buy peak across active holders and do not fall back to stale pre-exit metric values. Dashboard alerts table shows **Date** (`created_at`). `GET /api/alerts` / dashboard filter by active `profile_id`. Expiration: manual clear all + acknowledge (own alerts only); hourly 100h max age; new trading day after daily sync; **full sell** expires only that profile's alerts (`expireForProfileStockIfUnheld`). Active = `expired_at` IS NULL. Stale stoploss alerts are not auto-cleared when price recovers above the trailing stop.
