...
**UI:** Main nav **Screener** with tabs **My screens** | **Shared screens** | **Guide** (`/screeners`, `/screeners?tab=shared`, `/screeners?tab=guide`; `/screeners/new` create, `/screeners/:id` edit). My screens: TanStack DataTable (sort, columns, fit-to-window). Nested AND/OR condition builder; scope `holdings` | `watchlist` | `all_equities`; optional schedule + **Notify me results (only for cron runs)** when schedule is enabled. Create/edit toggle **Share with other portfolios** (`is_shared`, default off). Shared tab: other portfolios’ shared screeners + **Import** (private copy, schedule off; watchlist scope → holdings). Guide tab: scopes, operators, lookback, sharing, indicator catalog. Editor has live inline validation while typing (name required + max 120 + allowed chars, description max 500 + allowed chars, watchlist/time checks) and Save/Run stay disabled while invalid. Missing/deleted watchlist scope shows red field error on edit; list **Last run** column shows counts plus ⚠ warning text from run stats (e.g. deleted watchlist empty set). Editor footer uses **Close**; save toasts include screener name.

**Tables:** `portfolio_screeners`, `portfolio_screener_runs`, `portfolio_screener_run_hits` (migrations `2026_07_20_000001`, `2026_07_20_000002` adds `is_shared`). Scoped by `profile_id` (same as watchlists/alert policies — portfolios belong to users; API route binding uses `activePortfolio()` so screeners do not bleed across users). Hard-delete screener cascades runs/hits.

**Engine:** `App\Services\Screener\TechnicalIndicatorService` + `ScreenerEvaluationService` + `ScreenerRunService`. Indicators: close/OHLCV, change_pct, high_n/low_n, **high_52w/low_52w** (252-session rolling high/low), range_pct, sma/ema (+ vs price / spreads), rsi, roc, stoch_k/d, macd/signal/hist, atr, bollinger set, volume_sma/ratio. Period-style params allow **minimum 1** (e.g. SMA/EMA period 1 = latest close). **Min sessions** per stock = max lookback derived from your condition params (not a fixed 20). Insufficient bars → stock skipped (`skipped_insufficient_data`), run still completes. Universe runs chunked (~150 stocks) with `POST /api/screener-runs/{id}/continue`.

**API:** `GET/POST /api/screeners`, `GET /api/screeners/shared`, `POST /api/screeners/shared/{id}/import`, `GET/PUT/DELETE /api/screeners/{id}`, `GET /api/screeners/meta`, `POST /api/screeners/{id}/run`, `GET /api/screeners/{id}/runs`, `DELETE /api/screeners/{id}/runs` (clear all run history + hits), `GET /api/screener-runs/{run}`, `POST /api/screener-runs/{run}/continue`.
...
**Tests:** `tests/Unit/Screener/TechnicalIndicatorServiceTest.php`, `tests/Feature/ScreenerTest.php`.

**Deploy:** run migration via `cpanel-migrate.php` after upload.
