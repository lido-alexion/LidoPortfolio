...
**UI (2026-07-30):** Primary nav is a grouped **sidebar** (not horizontal tabs). Removed footer bottom nav. Notification history remains under Portfolio settings.
...
- **Detection pipelines:** new commands/services:
  ...
  - `portfolio:auto-resolve-data-quality-issues` auto-accepts pending issues older than 15 days (resolution retained as audit event).
...
- Shell UI: full-bleed black header (`AppHeader.jsx`, brand **StoX** in **Nulshock** at the same size as the former “Lido Alexion” title, with smaller cursive **by Lido Alexion** byline; font via `resources/fonts/nulshock-bd.ttf`, bundled into `build/assets/` with **relative** `url(./nulshock-….ttf)` in `lido-app.css` so the font loads under `/portfolio/build/assets/` even if `npm run build` omits `VITE_APP_BASE`), **sidebar toggle** (desktop collapse / mobile drawer), **portfolio switcher**, **contextual help** (`HeaderHelpButton.jsx` — (?) left of profile; maps current `pathname` → keyword via `utils/documentationLinks.js` + `data/appDocumentation.js`, opens `/documentation?q=<keyword>` in a new tab), profile menu (`ProfileMenu.jsx`, includes `ThemeToggle.jsx` 3-segment sun/monitor/moon theme switch; account name links to `/profile`; avatar shows uploaded photo or initial; display name = trimmed `name` or email). **Primary navigation (2026-07-30):** left sidebar (`AppSidebar.jsx` + `config/mainNav.js` `PRIMARY_NAV_SECTIONS`) — Portfolio (Dashboard, Portfolio, Transactions, Cash, Watchlist, Corporate Actions), Market (Stock Explorer, Discovery, Indices, Market Depth, Calendar, Patterns), Trading (Recommendations, Screeners, Strategy, Review), Knowledge (Knowledge Board), Administration (Settings only). Child routes (pending execution, closed transactions, stock prices, screener/strategy editors & registries, knowledge tags, all Settings sub-pages) stay off the sidebar and are reached from parents / Settings. Routes and page behaviour unchanged. Desktop sidebar collapse preference: `localStorage` `lido-sidebar-collapsed`. Footer bottom nav removed (was duplicating primary links). **Documentation page** (`/documentation`, `DocumentationPage.jsx`): searchable topic index; loads article from `?q=` (keyword / alias / fuzzy); each topic covers page overview, controls, concepts, and related links. Sidebar hidden on documentation. **Profile page** (`/profile`, `ProfilePage.jsx`): change display name, read-only username (email), change password, profile photo (JPEG/PNG only, max 5 MB) — large clickable circle (360px) opens file picker on click with tooltip; upload shows spinner overlay; **Remove photo** bright-red text link centered on hover when a photo is set. API: `GET/PUT /api/profile`, `PUT /api/profile/password`, `GET/POST/DELETE /api/profile/photo`; photos in `storage/app/profile-photos/` served only to authenticated user via session cookie. **Themes:** `light` / `dark` / `system` via `ThemeContext` + CSS variables in `lido-app.css` (`localStorage` key `lido-theme`); `app.blade.php` inline script avoids flash. Dark theme: `table-striped`, `table-hover`, and `.pagination` use Lido CSS variables (readable text on striped/hover rows; themed pagination on sync logs). Dev: `npx vite` + `@viteReactRefresh`; prod: `npm run build`.
