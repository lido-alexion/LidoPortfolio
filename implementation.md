...
- **Dashboard client cache (Jul 2026):** `GET /dashboard` + `GET /patterns/scan` responses cached in `localStorage` per user + active portfolio (`utils/dashboardCache.js`, key `portfolio_dashboard_cache_v1_{userId}_{profileId}`). **24h TTL**; unused `nifty_comparison.prices` stripped before store. On revisit within TTL, dashboard renders instantly with no API calls. **Refresh dashboard** button clears cache and refetches. Cache invalidated on logout (`clearAllDashboardCaches`), transaction mutations (`notifyPortfolioDashboardRefresh`), and portfolio switch (`notifyPortfolioChanged`). Dashboard mutations (acknowledge/clear alerts, sync, rebuild) clear cache and refetch. Shows muted **Cached {time}** label when serving from cache.
...
- Holdings table default column order: Stock → Latest Close → **Unrealized P/L** → Invested → Fees → XIRR → Highest Close → **Qty** → **Avg Buy** → Trailing Stop → Realized P/L → OHLCV → Sell. **Fees** and **Realized P/L** hidden by default (`defaultColumnVisibility`); column prefs stored per view in `localStorage` keys `portfolio_datatable_holdings` (complex) and `portfolio_datatable_holdings-simple` (simple). **Simple / Complex** toggle in card header (before columns menu): complex = current multi-line cells; simple = primary value only (no since-buy date, price date, daily % in Latest Close, unrealized % subline, fee %, LTP drawdown, stop %, OHLCV row count, etc.). Selected view persisted in `portfolio_holdings_view`. Two table instances share the same dataset; only the active view is shown; column menu applies to the visible view only.
...
**Automated normal-day maintenance (Jul 2026):** Scheduler now runs **`portfolio:run-universe-maintenance`** every **15 minutes** 19:00–23:45 (when universe sync is enabled). Logic per run: (1) execute one universe **daily** batch, (2) if `failed > 0`, auto-run gap fill batch **2 times** with wait between retries (default 20s). Configurable via env: `UNIVERSE_MAINTENANCE_GAP_FILL_RETRIES` (default 2), `UNIVERSE_GAP_FILL_WAIT_SECONDS` (default 20). Command supports overrides: `--gap-retries`, `--gap-wait-seconds`.
...
| `portfolio:run-universe-maintenance`                   | Daily batch + auto gap-fill retries when failures occur                |
...
2. `POST /api/analytics/explore` → `ExploratoryAnalyticsService::analyze()` — always **1, 3, and 6 months**; benchmark NIFTY50.
...
5. Response includes `latest_close`, `period_closes.{1m,3m,6m}`, `growth_percent`, `benchmark_growth_percent`, `relative_strength`, and Recharts comparison chart (3 x-axis groups × 2 bars = **6 bars**: stock + NIFTY growth per period).
6. UI shows latest close for stock and NIFTY50, plus **three RS cards** (1M / 3M / 6M) color-coded green/red; growth comparison chart shows all three periods at once.
