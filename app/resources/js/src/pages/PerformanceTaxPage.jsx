    ...
    const preserve = async (calculationType) => {
        ...
        try {
            const payload = calculationType === 'portfolio_performance'
                ...
                : calculationType === 'account_performance'
                    ? { calculation_type: calculationType, from, to, ...(whatIf ? { portfolio_ids: selectedIds } : {}) }
    ...
    const savePreferences = async () => {
        ...
        try {
            await api.put('/analysis/preferences', {
                ...
                comparison_benchmark_ids: preferenceForm.comparison_benchmark_ids ?? [],
    ...
    const saveAccountPreferences = async () => {
        ...
        try {
            await api.put('/analysis/preferences', {
                ...
                comparison_benchmark_ids: accountPreferenceForm.comparison_benchmark_ids ?? [],
    ...
    return <div className="container-fluid py-3">
        ...
        {data ? <>
            {preferenceForm ? <div className="card mb-3"><div className="card-header">Analysis settings · {activePortfolio.name}</div><div className="card-body row g-2 align-items-end">
                ...
                <div className="col-md-4"><div className="small mb-1">Comparison benchmarks</div>{data.benchmarks.filter((benchmark) => benchmark.id !== Number(preferenceForm.primary_benchmark_id)).map((benchmark) => <label className="d-block small" key={benchmark.id}><input className="form-check-input me-1" type="checkbox" checked={(preferenceForm.comparison_benchmark_ids ?? []).includes(benchmark.id)} onChange={(event) => setPreferenceForm((value) => ({ ...value, comparison_benchmark_ids: event.target.checked ? [...(value.comparison_benchmark_ids ?? []), benchmark.id] : (value.comparison_benchmark_ids ?? []).filter((id) => id !== benchmark.id) }))} />{benchmark.name}</label>)}</div>
            ...
            <div className="card mb-3"><div className="card-header d-flex justify-content-between align-items-center"><span>Account performance · {data.accountPerformance.calculation_mode}</span><button type="button" className="btn btn-sm btn-outline-secondary" disabled={savingEvidence} onClick={() => preserve('account_performance')}>Preserve evidence</button></div><div className="card-body small">Independently aggregated across {data.accountPerformance.portfolio_ids.length} included portfolio(s), never averaged: XIRR {data.accountPerformance.xirr_percent == null ? 'Incomplete' : formatTablePercent2(data.accountPerformance.xirr_percent)} · TWR {data.accountPerformance.twr_percent == null ? 'Incomplete' : formatTablePercent2(data.accountPerformance.twr_percent)}.</div></div>
