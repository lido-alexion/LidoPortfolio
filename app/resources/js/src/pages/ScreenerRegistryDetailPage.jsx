 ...
 */
export default function ScreenerRegistryDetailPage({ adminMode = false }) {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setError('');
            try {
                const showRes = await api.get(`/v1/screener-registry/${encodeURIComponent(id)}`);
                if (cancelled) return;
                const data = showRes.data?.data || null;
                setEnv(data);
                const own = data?.metadata?.ownership !== 'shared';
                if (own) {
                    try {
                        const verRes = await api.get(`/v1/screener-registry/${encodeURIComponent(id)}/versions`);
                        if (!cancelled) setVersions(verRes.data?.data || []);
                    } catch {
                ...
                if (!cancelled) {
                    setError(err?.response?.data?.error?.message || err.message || 'Failed to load screener');
                    setEnv(null);
                }
            } finally {
    ...
    const isShared = meta.ownership === 'shared';

    return (
        <div className="d-grid gap-3">
            <div className="d-flex flex-wrap justify-content-between align-items-start gap-2">
                <div>
                    <h2 className="h4 mb-1">{env?.name || 'Screener artifact'}</h2>
                ...
                <div className="d-flex flex-wrap gap-2">
                    ...
                    {!isShared && env?.artifact_id && (
                        <Link to={`/screeners/${encodeURIComponent(env.artifact_id)}`} className="btn btn-outline-primary btn-sm">
                            Open editor
                        </Link>
                    )}
                    <button type="button" className="btn btn-primary btn-sm" onClick={onExport} disabled={!env}>
                        Export JSON
                    </button>
                </div>
            </div>
            ...
            {!loading && env && (
                <>
                    <div className="card">
                        <div className="card-body">
                            <dl className="row mb-0 small">
                                ...
                                <dd className="col-sm-9">{meta.status || '—'} · {meta.origin || '—'} · {meta.ownership || 'own'}</dd>
                                <dt className="col-sm-3">Universe</dt>
                                <dd className="col-sm-9"><code>{meta.universe || '—'}</code></dd>
                                ...
                                <dd className="col-sm-9">
                                    {(env.dependencies || []).length
                                        ? (env.dependencies || []).map((d) => d.ref).join(', ')
                                        : '—'}
                                </dd>
                            </dl>
                        </div>
                    ...
                    <div className="card">
                        <div className="card-body">
                            <h3 className="h6">Definition (condition tree)</h3>
