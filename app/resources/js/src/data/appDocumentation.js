...
const APP_DOCUMENTATION_BASE = [
    ...
    },
    {
        ...
        overview:
            ...
            + '```\n\n'
            + 'Configure Screeners + Strategy first, then run the decision pipeline from Recommendations (or the scheduled daily pipeline). Current behaviour is summarized in docs/current/strategy-and-recommendations.md.',
        controls: [
            {
    ...
    },
    {
        ...
        concepts: [
            ...
            { name: 'Kite readiness', description: 'Automatic mode only. Checked independently of the Dashboard cache. If the session is missing or expired, Connect Kite starts Kite authentication and returns to Dashboard afterward. Enter any Kite login OTP only on the Kite login page.' },
    ...
    },
    {
        ...
        overview:
            'After you Approve a BUY or SELL recommendation, it moves here as pending execution. In Manual mode, you trade at your broker then record the fill in the app (ledger transaction). In Semi-Automatic mode, selecting rows is not execution — use Accept / Execute Selected with your StoX execution code — {Authenticator App Name} to submit to Kite. Automatic mode submits eligible orders without a per-order click. Broker acceptance is not a fill. An actionable broker Recommendation seeks its monetary target over its next two eligible NSE sessions, can make partial progress, and completes only when fills or internal transfers reach that target.',
        controls: [
            ...
            { name: 'Accept / Execute Selected', description: 'Semi-Automatic only. Submits selected eligible recommendations to Kite after server checks (entitlement, StoX execution code, capital, broker session). Not shown as a substitute for Automatic mode.' },
            { name: 'StoX execution code — Authenticator app', description: 'Rotating code from the named authenticator app. Required on Semi-Automatic submit and Emergency Halt recovery; not required for Manual ledger fills.' },
            { name: 'Cancel execution', description: 'Drops pending status and releases any reserved cash for buys. Does not cancel an in-flight Kite order by itself.' },
        ...
        concepts: [
            ...
            { name: 'Two-session target seeking', description: 'The target amount is authoritative; displayed quantity is only a stored-close estimate. StoX recalculates the remaining whole-share quantity before each attempt, can progress by one share, and expires only the unresolved portion at the second eligible session cutoff. Weekends and exchange Trade Holidays do not count as opportunities.' },
            { name: 'Investor-level coordination', description: 'Automatic intents across your portfolios share one Kite-account cycle. An authenticated Semi-Automatic action adds only its selected approved intents. Opposing same-symbol Strategy intents transfer ownership internally first; only the residual reaches Kite, with residual sells processed before buys and buys bounded by currently reported shared Kite funds.' },
        ],
    ...
    },
    {
        ...
        overview:
            'Settings is the only Administration item in the primary sidebar. Open it for Global (admins), Portfolio, and Account preferences. Portfolio → Execution mode chooses Manual / Semi-Automatic / Automatic for the active portfolio. Account → StoX execution code setup and Zerodha/Kite connection are required before Semi-Automatic or Automatic broker submission. On Account, Active sessions lists your signed-in devices so you can revoke one session or log out other devices. Admin tools (users, stocks catalogue, sync logs, data quality, indicator/screener/strategy registries, universe sync, alert policies) are linked from Settings cards — they are not separate sidebar entries.',
        controls: [
            { name: 'Settings tabs', description: 'Navigate Global / Portfolio / Account sections.' },
            { name: 'Execution mode', description: 'Per active portfolio. New portfolios default to Manual. Semi-Automatic and Automatic require admin entitlement plus a valid StoX execution code — {Authenticator App Name}. Manual → Automatic needs an explicit confirmation checkbox. Automatic → Manual stops future automatic submits and does not cancel broker orders already sent. Automatic mode is unattended in production: after the scheduled daily pipeline, eligible orders submit without a per-order click (Laravel schedule:run; existing StoX execution-code setup and Kite session are still required).' },
            { name: 'StoX execution code', description: 'Account tab. Enroll Google Authenticator, Microsoft Authenticator, or another app and give it a display name. Confirm setup with the rotating StoX execution code — {Authenticator App Name}. StoX recovery codes are separate one-time backup codes shown once. Required before broker submission in Semi-Automatic or Automatic and for Emergency Halt recovery; not required for Manual fills. Disable requires a current StoX execution code — {Authenticator App Name}. Kite login OTPs are entered only on Kite/Zerodha.' },
            { name: 'Zerodha / Kite', description: 'Account tab. Connect your own Kite account. Not required for Manual mode. Sessions typically expire around 6:00 AM IST; reconnect if broker submit is blocked. Portfolio settings can schedule one daily Telegram readiness reminder for Automatic mode; clear its time to disable.' },
