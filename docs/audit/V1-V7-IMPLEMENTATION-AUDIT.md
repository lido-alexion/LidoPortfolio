...
| AUD-012 | V5-REQ-018 | Authorization/security | Admin and investor applications expose only intended routes/data and preserve ownership | Production ownership audit is clean; AUTHR-001 is implemented with effective production DebugAgent disabled, a production hard block, and deployment/health gates. AUTHR-002 remains open until an administrator repairs existing dated Laravel log ownership for PHP-FPM; the current route inventory is 443 versus the prior 437-operation audit, and a full authenticated foreign-object runtime matrix remains unavailable. | PARTIALLY_IMPLEMENTED | High | High | V5 FEAT-042 | `App.jsx`; `AdminRoute`; `ResolveActivePortfolio.php`; `PortfolioProfile.php`; auth middleware/controllers; `AUD-012-AUTHORIZATION-OWNERSHIP-RUNTIME-VERIFICATION.md` | `RoleSeparatedApplicationTest`, `PortfolioMiddlewareTest`, `V6PersonalApiTokenTest`, `V6ContextualNotesTest`, `KiteCallbackTest`, `DebugAgentTokenTest`, `AdminInvestmentOwnershipAuditTest`; production ownership command and route inventory | Repair PHP-FPM log permissions and complete clean role/foreign-object runtime probes | Apply administrator storage repair, then rerun protected unauthenticated denial checks and approved two-actor/PAT verification | AUD-005, AUD-009 | AUTHR-001 closed; retain AUTHR-002 and route/object runtime boundaries. |
