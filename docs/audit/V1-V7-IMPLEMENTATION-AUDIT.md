...
| V5-REQ-010 | Production secrets/single-folder deploy | RUNTIME_VERIFICATION_REQUIRED | VPS release/shared deployment, root-managed queue unit, FPM release-identity gate, and deployment docs | AUD-009 production evidence proves secret/public-root isolation, FPM activation, named queue recovery, and scheduler health | Approved newer release/migration freshness; backup retention and restore assurance | Yes | `AUD-009`; DEP-001/DEP-002 implemented, DEP-003/DEP-004 remain |
...
| V7-REQ-003 | `stox_` namespace enforcement | PARTIALLY_IMPLEMENTED | The governed post-cutoff migration inventory now contains 10 canonical `stox_` objects; legacy `portfolio_*` tables remain by design | Cutoff-wide `StoxNamespaceValidationTest`, full migration inventory, and read-only production schema inventory | Production still has the two post-cutoff historical Replay tables under legacy names until the forward rename migration deploys | Yes | Repository enforcement is complete; deploy and verify the two-table compatibility rename before closure |
...
| AUD-009 | V5-REQ-010 | Deployment/secrets | Production uses the documented VPS release/shared secret/deploy arrangement | Production verification now proves secret/public-root isolation, scheduler health, FPM release activation, and a root-owned `notifications,default` worker. The 12-job recovery completed as 11 delivered and 1 suppressed with 0 failed jobs. DEP-003 current-source freshness and DEP-004 backup/restore evidence remain. | RUNTIME_VERIFICATION_REQUIRED | Medium | High | V5 FEAT-031 and deployment docs; `AUD-009-DEPLOYMENT-RUNTIME-VERIFICATION.md` | `deploy/`, `DEPLOYMENT_*`, config/bootstrap | Deployment runtime health gate; production activation and queue-recovery evidence | Approved newer release/migration deployment; backup retention and restore drill | Verify approved release freshness and backup/restore evidence | V4-REQ-006, V7-REQ-001/002 | DEP-001/DEP-002 implemented; retain only DEP-003/DEP-004 runtime verification |
...
| AUD-014 | V7-REQ-003 | Database governance | New StoX objects use canonical namespace and automated checks prevent future drift | Complete migration inventory and cutoff-wide validator are implemented; two post-cutoff historical Replay tables were found under legacy names in production and a forward rename migration is prepared | PARTIALLY_IMPLEMENTED | Medium | High | V7 StoX Database Namespace Specification; `AUD-014-DATABASE-NAMESPACE-RUNTIME-VERIFICATION.md` | `database/migrations`; Replay evidence models; `tests/Unit/V7/StoxNamespaceValidationTest.php` | Namespace validator, Replay temporal tests, V7 fundamental/ML tests, production read-only schema inventory | Normal deployment of the rename migration and post-deploy schema verification | Deploy `2026_09_19_000001_v7_namespace_historical_replay_evidence.php`, then rerun inventory | AUD-009 | Legacy baseline is intentionally retained; only the two post-cutoff Replay objects require rollout |
