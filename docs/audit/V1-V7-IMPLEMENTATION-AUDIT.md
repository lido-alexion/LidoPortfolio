...
| AUD-003 | V6-REQ-010 | UX consistency | Standard page anatomy and component foundations adopted consistently | Batch 1 added the shared `DataState` primitive for loading/empty/error/unavailable/incomplete states, applied it to Candidates, Notification History and Review Reports, and migrated Candidates to the existing `DataTableView` controller path. Global page-anatomy adoption remains incomplete; UX-003 through UX-006 were not changed. | PARTIALLY_IMPLEMENTED | Medium | Medium | E4 component foundations and page anatomy sections | `PageChrome.jsx`; `components/DataState.jsx`; `components/DataTable.jsx`; `CandidatesPage.jsx`; `NotificationHistoryPage.jsx`; `ReviewReportsListPage.jsx` | `data-state.test.jsx`; Candidates and Review Reports TOS suites; full Vitest and Node/source suites pass | Remaining list-family adoption, page action/title consistency, scroll behavior, accessibility and browser responsive evidence | Desktop/mobile route sweep | AUD-001, AUD-002, AUD-013 | Batch 1 complete for representative UX-001 coverage; UX-002 remains partial. Continue with incremental page-family adoption and do not mark AUD-003 complete. |
