...
| AUD-008 | V5-REQ-008, V6-REQ-002 | Backtest/paper semantics | Backtest, replay and paper execution are distinct, safely isolated and reachable | Batch 1 proves historical Replay branch reconstruction, as-of holdings/cash/ownership, binding/version pins, capital-state components, source isolation, processed checkpoints, and precise fail-closed blockers. Batch 2 proves Level 1 post-resync guarantees: completed results/evidence remain immutable, while equivalent reruns read corrected current data; exact rerun reproducibility is not promised by the current contract | PARTIALLY_IMPLEMENTED | Medium | Medium | V5 FEAT-020 and V6 E2 specs | backtest pages/services/routes; `HistoricalReplayStateBuilder`; `HistoricalSimulationDatasetResyncTest` | Replay foundation, historical branch, and controlled resync assurance tests | UI semantics, deployed workers, and browser/runtime behavior; SIM-003 composite assurance remains optional | Run each workflow with representative portfolio and controlled resync | AUD-013 | SIM-001 implemented; SIM-002 implemented with limitation; SIM-003/SIM-004 remain bounded verification work |
