...
| AUD-003 | V6-REQ-010 | UX consistency | Standard page anatomy and component foundations adopted consistently | Batches 1–2 added shared state presentation, migrated Candidates to `DataTableView`, aligned representative heading hierarchy under `PageChrome`, and replaced Market Depth’s equivalent local two-option control with shared `SegmentToggle`. Global page-anatomy adoption remains incomplete; UX-005 and UX-006 were not changed. | PARTIALLY_IMPLEMENTED | Medium | Medium | E4 component foundations and page anatomy sections | `PageChrome.jsx`; `components/DataState.jsx`; `components/DataTable.jsx`; `components/SegmentToggle.jsx`; `CandidatesPage.jsx`; `NotificationHistoryPage.jsx`; `MarketDepthPage.jsx`; `ReviewReportsListPage.jsx` | `data-state.test.jsx`; `ux003-batch2.test.mjs`; Candidates and Review Reports TOS suites; full Vitest and Node/source suites pass | Remaining list-family adoption, action composition, scroll behavior, accessibility and browser responsive evidence | Desktop/mobile route sweep | AUD-001, AUD-002, AUD-013 | Batches 1–2 complete representative UX-001 coverage and targeted UX-003/UX-004 improvements. UX-002 remains partial; continue incrementally and do not mark AUD-003 complete. |
