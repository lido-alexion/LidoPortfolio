...
| AUD-006 | V5-REQ-002, V1-REQ-006 | Notifications | Critical persistence/read semantics, channel health, retry and reminder behavior are reachable and clear | Batches 1–4 now cover the current source/occurrence/recipient/delivery lifecycle, safe per-channel delivery detail and retry, unknown-versus-zero bell/settings states, validated primary-action navigation, and explicit initial/escalation/reminder labels. Read, acknowledgement, resolution, retry, and reminder remain separate semantics. | IMPLEMENTED | Medium | High | `V5-FEAT-004-Notification-Service.md` | notification components, routes, services, delivery/reminder services | Notification lifecycle, delivery API/retry, shell/source, settings, publisher/planner/processor/reminder tests | Real queue worker, scheduler, provider credentials/network, deployed provider behavior, browser runtime verification | Deployment/runtime verification for notifications and reminders | AUD-010 | Static remediation complete. Verify queue/scheduler/provider delivery and deployed browser behavior; retain NTF-005 runtime boundary |
