...
| AUD-005 | V3-REQ-010, V4-SPEC-001 | Multi-strategy UX / ownership | Users can discover and operate strategy-owned/unmanaged holdings, capital badges, lending/recall and lifecycle controls | Batch 1 now proves the backend/application MS-001 lifecycle: two enabled strategies, 60/40 allocation, one physical cash pool, Strategy A funding/reservation, approval-vs-execution separation, deterministic execution, transaction/holding attribution, sibling same-stock isolation, unmanaged adoption/merge, reservation release, and representative cross-profile rejection. | PARTIALLY_IMPLEMENTED | High | Medium | V3 specification §§1-2 and V4 adoption decisions | `MultiStrategyLifecycleAssuranceTest`; `RecommendationLifecycleService`; `ExecutionEngine`; `PortfolioCapitalAccountingService`; `HoldingAdoptionService`; `/api/holdings/{holding}/adopt`; `/api/v1/recommendations/{id}/review` | `MultiStrategyLifecycleAssuranceTest` (5 tests/52 assertions), existing ownership/capital/lending/execution suites | Full Investor browser journey, ownership naming/provenance explanation, allocation-change explanation, and degraded-state recovery | Multi-portfolio runtime workflow | AUD-003 | MS-001 is executable-assured; retain MS-002 through MS-005 and do not infer overall closure |
