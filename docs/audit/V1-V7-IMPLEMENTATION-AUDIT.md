...
The repository has substantial domain, API, and test coverage, but that is not equivalent to full product-contract coverage. AUD-001 and AUD-002 are now remediated statically: the Investor shell mounts session-backed Page Visit History and a coordinated Contextual Notes utility with focused state, accessibility and router tests. The shell still has no Global Search implementation or mounted low-priority footer, and browser geometry/deployment checks remain separate runtime work.
...
| V6-REQ-007 | Global shell: header, left nav, workspace, right rail/pane | PARTIALLY_IMPLEMENTED | `AuthenticatedShell` mounts `AppHeader`, `Sidebar`, `.lido-main` and the Investor `RightUtilityRail`, which coordinates Page History and Notes | `roleSeparatedShell.test.mjs` inspects shell source; Page History and Contextual Notes shell tests cover utility behavior | No Global Search component/state; no mounted low-priority footer; exact constrained-width geometry requires browser inspection | Yes | `AUD-003`, `AUD-016`; shell-level runtime verification remains |
...
- **Shell contract versus implementation:** current frontend documentation correctly retains the V6 E4 rail, Page History, Global Search and footer contract. The mounted React shell now implements Page History and the Notes rail statically; Global Search and footer remain separate alignment items (`AUD-016` and the combined shell verification in `AUD-003`).
...
No meaningful automated test covers the complete V6 E4 header Global Search contract. `page-history.test.jsx` proves Page History state, persistence validation, active links, client-side navigation and mobile dialog behavior; it does not prove desktop/mobile CSS geometry, touch layout, stacking or deployed-bundle reachability. `contextual-notes-shell.test.jsx` now proves the Notes rail/sheet state, focus and route-context behavior, while `V6ContextualNotesTest` proves personal/profile-scoped CRUD. Neither proves browser geometry, touch/soft-keyboard behavior, deployed reachability or entity subject resolution. The following accepted contracts also have insufficient end-to-end test evidence:
...
2. **V6-REQ-007:** Header, collapsible sidebar, main workspace, Page Visit History and the coordinated Notes utility are mounted. Global Search and a mounted footer zone remain absent; exact shared-shell geometry still needs browser verification.
