...
| V5-REQ-018 | Role-separated Admin/Investor apps | PARTIALLY_IMPLEMENTED | `AdminAppRoutes`, `AdminRoute`, role branches in `App.jsx` | AuthZ/admin tests; production ownership audit is clean; DebugAgent and Laravel logging runtime defects are closed | Route inventory delta and complete authenticated role/object runtime matrix remain open | Yes | `AUD-012` |
...
| AUD-012 | V5-REQ-018 | Authorization/security | Admin and investor applications expose only intended routes/data and preserve ownership | Production ownership audit is clean; AUTHR-001 and AUTHR-002 are implemented with production DebugAgent disabled, hard deployment/runtime gates, repaired shared Laravel logging, a 401 protected API denial, and green runtime health. The route inventory is 443 versus the prior 437-operation audit, and a full authenticated foreign-object runtime matrix remains unavailable. | PARTIALLY_IMPLEMENTED | High | High | V5 FEAT-042 | `App.jsx`; `AdminRoute`; `ResolveActivePortfolio.php`; `PortfolioProfile.php`; auth middleware/controllers; `AUD-012-AUTHORIZATION-OWNERSHIP-RUNTIME-VERIFICATION.md` | `RoleSeparatedApplicationTest`, `PortfolioMiddlewareTest`, `V6PersonalApiTokenTest`, `V6ContextualNotesTest`, `KiteCallbackTest`, `DebugAgentTokenTest`, `AdminInvestmentOwnershipAuditTest`; production ownership command, route inventory, release `6838f8277a79a3cd36996f6f50b33dd559915caa`, and runtime health check | Review route delta and complete authenticated role/foreign-object runtime probes | Perform AUTHR-005/006 route and two-actor/PAT verification | AUD-005, AUD-009 | AUTHR-001/AUTHR-002 closed; retain AUTHR-005/AUTHR-006. |
