...
| AUD-005 | V3-REQ-010, V4-SPEC-001 | Multi-strategy UX / ownership | Users can discover and operate strategy-owned/unmanaged holdings, capital badges, lending/recall and lifecycle controls | Batch 1 proves the backend/application MS-001 lifecycle. Batch 2 additively exposes resolved strategy metadata, keeps same-stock ownership episodes as separate Holdings rows, preserves archived-owner identity, and explains destination weighted merge versus sibling isolation in the adoption modal. | PARTIALLY_IMPLEMENTED | High | Medium | V3 specification §§1-2 and V4 adoption decisions | `HoldingController.php`; `HoldingPresentationService.php`; `HoldingsPage.jsx`; `holdingOwnership.js`; `/api/holdings`; `/api/holdings/{holding}/adopt` | `HoldingsOwnershipPresentationTest` (2 tests), `HoldingAdoptionTest`, `holdings-ownership.test.jsx` source/helper coverage, `MultiStrategyLifecycleAssuranceTest` | Allocation-change explanation and full funding/lending/error recovery presentation; browser verification of discoverability and responsive ownership detail | Multi-portfolio runtime workflow | AUD-003 | MS-001, MS-002 and scoped MS-003 ownership/provenance UX are assured; retain MS-004/MS-005 and overall partial status |
