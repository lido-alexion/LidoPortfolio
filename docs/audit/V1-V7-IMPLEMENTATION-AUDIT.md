...
| AUD-005 | V3-REQ-010, V4-SPEC-001 | Multi-strategy UX / ownership | Users can discover and operate strategy-owned/unmanaged holdings, capital badges, lending/recall and lifecycle controls | Batch 1 proves the backend/application MS-001 lifecycle. Batch 2 exposes resolved strategy metadata, preserves separate same-stock Holdings rows and archived-owner identity, and explains destination weighted merge versus sibling isolation. Batch 3 proves allocation changes are policy-only and adds derived allocation variance/status plus explicit above-allocation explanation. | PARTIALLY_IMPLEMENTED | High | Medium | V3 specification §§1-2 and V4 adoption decisions | `PortfolioCapitalAccountingService.php`; `CashManagementPage.jsx`; `strategyAllocationStatus.js`; `/api/v1/capital/allocations`; `HoldingController.php`; `HoldingPresentationService.php`; `HoldingsPage.jsx` | `MultiStrategyLifecycleAssuranceTest` (6 tests/63 assertions), `HoldingsOwnershipPresentationTest`, capital/lending/proceeds suites, ownership/adoption suites; frontend helper coverage added | Full MS-005 missing/stale/error recovery presentation; browser verification of allocation explanation and responsive detail | Multi-portfolio runtime workflow | AUD-003 | MS-001 through MS-004 are assured for their scoped boundaries; retain MS-005 and overall partial status |
