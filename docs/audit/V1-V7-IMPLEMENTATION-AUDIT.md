...
| AUD-012 | V5-REQ-018 | Authorization/security | Admin and investor applications expose only intended routes/data and preserve ownership | Production ownership audit is clean; static role/profile/PAT/object enforcement remains substantial, but production has the pre-launch DebugAgent hook enabled and protected unauthenticated APIs return 500 because PHP cannot write the current Laravel log. The current route inventory is 443 versus the prior 437-operation audit, and a full authenticated foreign-object runtime matrix remains unavailable. | PARTIALLY_IMPLEMENTED | High | High | V5 FEAT-042 | `App.jsx`; `AdminRoute`; `ResolveActivePortfolio.php`; `PortfolioProfile.php`; auth middleware/controllers; `AUD-012-AUTHORIZATION-OWNERSHIP-RUNTIME-VERIFICATION.md` | `RoleSeparatedApplicationTest`, `PortfolioMiddlewareTest`, `V6PersonalApiTokenTest`, `V6ContextualNotesTest`, `KiteCallbackTest`, `DebugAgentTokenTest`, `AdminInvestmentOwnershipAuditTest`; production ownership command and route inventory | Disable debug authentication, correct PHP-FPM log permissions, and complete clean role/foreign-object runtime probes | Remediate deployment auth/log boundary, then run approved two-actor/PAT verification | AUD-005, AUD-009 | Confirmed deployment authorization defect; do not treat clean ownership data as closure while DebugAgent remains enabled. |
