...
| AUD-005 | V3-REQ-010, V4-SPEC-001 | Multi-strategy UX / ownership | Users can discover and operate strategy-owned/unmanaged holdings, capital badges, lending/recall and lifecycle controls | Batches 1–4 prove strategy provenance, separate ownership episodes, weighted adoption, policy-only allocation changes, derived above-allocation state, and distinct missing/stale/blocked/partial/failed state presentation across Holdings, Recommendations, Cash Management, lending/recall, and sale proceeds. Missing quote data remains unavailable rather than zero; failed list/snapshot requests remain errors rather than empty success. | IMPLEMENTED | High | Medium | V3 specification §§1-2 and V4 adoption decisions | `PortfolioCapitalAccountingService.php`; `StockQuoteService.php`; `HoldingPresentationService.php`; `CashManagementPage.jsx`; `CapitalRecallPanel.jsx`; `RecommendationsPage.jsx`; `capitalRecallUi.js`; Holdings ownership files | `MultiStrategyLifecycleAssuranceTest`; `HoldingsOwnershipPresentationTest`; capital/lending/proceeds/recommendation lifecycle suites; state-semantics and ownership frontend tests; full Vitest | Browser wording/geometry, responsive recovery, deployed-bundle reachability, and real provider/automation scenarios | Multi-portfolio runtime workflow | AUD-003 | MS-001–MS-004 implemented; MS-005 implemented with runtime verification retained. No known unsafe missing→zero or failed→empty multi-strategy state remains. |
...
5. `AUD-005`: multi-strategy ownership/adoption/lending investor surfaces are not fully traceable end to end.
6. `AUD-014`: namespace enforcement is partial and lacks a complete drift inventory.
