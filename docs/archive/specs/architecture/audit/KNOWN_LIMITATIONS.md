...
10d. **Trading Artifact Framework rollout/final audit remains open in V5 (V4-FEAT-008)** (updated 2026-09-09) — Immutable lifecycle/distribution, exact Portfolio runtime resolution, legacy identity backfill/projection, mapped compatibility guards, artifact-version/binding-revision evidence through Screener runs and Recommendation/order/fill execution, immutable resumable backtest snapshots, and the authoring cutover to non-runnable Library Drafts are implemented. Representative-data rollout and the final full-suite/frozen-criteria audit remain to be closed. See [`../../V5-FEAT-008-IMPLEMENTATION-STATUS.md`](../../V5-FEAT-008-IMPLEMENTATION-STATUS.md).
