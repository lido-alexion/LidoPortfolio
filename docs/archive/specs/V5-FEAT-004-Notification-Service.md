...
**Status:** FROZEN / IMPLEMENTATION COMPLETE (verified 2026-09-08)
...
The shared `/api/notification-settings` foundation represents In-app as mandatory and stores Telegram, Email and Webhook configuration encrypted per account. External channels cannot be enabled before successful verification; material destination changes clear verification and disable delivery. Webhook URLs require HTTPS and receive a generated signing secret that is never returned by normal settings responses. The account login email is seeded as the mandatory Email destination. The role-shared Notification Settings page now exposes configuration, enablement, health and real-channel Test actions while preserving encrypted credentials when a user edits non-secret fields. Additional email recipients now have account-scoped add/list/remove APIs and signed expiring verification links; only verified destinations enter the delivery planner.
...
Notification publication now atomically plans a durable external-delivery outbox for each eligible recipient, enabled/verified channel and verified destination. Destinations are encrypted at rest and keyed by a non-secret hash; idempotency keys prevent duplicate initial or escalation delivery work. Info remains In-app only unless explicitly excepted, ordinary Active redetection does not resend, and escalation to Critical creates one separate delivery generation.

New outbox rows dispatch unique queue jobs after transaction commit. The production adapters deliver through Telegram, individually addressed Email or the versioned signed Webhook contract. Before network access the processor suppresses stale work for resolved conditions while retaining genuine event delivery. Retryable network/408/429/5xx failures use bounded queue backoff; permanent 4xx and exhausted failures stop. Every attempt records only sanitized status/error codes and HTTP status, and success advances the recipient notification's last-successful-external-delivery clock.

The hourly `portfolio:queue-notification-reminders` scheduler queues one idempotent reminder after 48 hours from the last successful external delivery while the condition remains Active and Action-required/Critical. Persistent or exhausted delivery failure creates one deduplicated Action-required `notification.channel_health` condition, excludes the failing channel from its own health delivery, and never recursively creates health notifications for health-condition failures. A successful later delivery or explicit channel disablement resolves the health condition. The daily Kite-readiness reminder and India VIX threshold alert use stable FEAT-004 Action-required conditions, including automatic resolution/re-arming when their underlying conditions clear. Scheduled screener completion summaries and calendar reminders publish channel-neutral FEAT-004 events with portfolio/event context.

No application producer now calls the legacy direct Telegram send methods. They remain temporarily as unused compatibility helpers while legacy Portfolio settings are readable for one-way account-channel migration.

Legacy portfolio-level Telegram configuration now has a compatibility bridge into the account-level channel model. When an account has one consistent complete credential pair across its active Portfolios, the bridge migrates it once into encrypted, verified account settings and preserves the effective enabled state. It runs before delivery planning and when Notification Settings is opened. If an account has conflicting credential pairs, the bridge deliberately makes no selection; the user must configure the account-level destination explicitly.

Scheduled active-alert digests now publish one Action-required FEAT-004 event per eligible Portfolio schedule instead of calling Telegram directly. Each event retains the Portfolio, scheduled time, alert count and contributing alert IDs, routes the recipient back to Dashboard alerts, and plans delivery through every enabled verified account channel. Repeated configured schedule times intentionally remain separate events so the shipped repeat-digest behavior is preserved. The optional no-alert clear ping remains on its legacy Telegram-only path until its global operational setting is reconciled with account-level channel semantics.

Stock-price provider and daily-market-sync failure hooks now publish Critical FEAT-004 events to Admin accounts through the durable outbox. The compatibility method name remains temporarily so existing job and provider call contracts do not churn, but it no longer broadcasts internal provider failure details to every Investor Portfolio with Telegram credentials. Accounts without an Admin do not create recipient-less notification sources.

Admin operational alert fan-out now uses FEAT-004 account channels through the existing compatibility method. Warning definitions map to Action-required events, Critical definitions remain Critical, and the explicit all-clear test ping is an externally delivered Info event. Admin-only recipient filtering, the existing six-hour repeat/cooldown decision and the operational-alert persistence model remain unchanged. A cooldown timestamp advances only when at least one external delivery row is planned, so adding a channel after a no-channel attempt is not suppressed. The reported Telegram-recipient count now reflects enabled, verified account settings (including safe legacy migration) rather than scanning Portfolio settings directly.

The Trading OS `NotificationEngine` now preserves `TosNotification` as its legacy, Portfolio-scoped history and idempotency facade while routing actual delivery through FEAT-004. Each legacy row links to exactly one channel-neutral source; recommendation, execution and lending notifications can therefore fan out through every enabled account channel without duplicate Telegram sends. Telegram worker success, retryable failure and terminal failure mirror back to the linked legacy row, and the legacy retry endpoint requeues the same failed outbox delivery rather than creating another source or history row. Existing payloads, notification types, recommendation links and idempotency keys remain intact.

The final Portfolio-scoped delivery exceptions are reconciled in favor of the later account-level FEAT-004 contract. Scheduled no-alert clear pings retain their explicit global opt-in and Portfolio schedule, but publish externally delivered Info events through account channels. The duplicate Telegram credential inputs and test button are removed from Portfolio Settings and replaced by a link to Notification Settings. The old authenticated test endpoint remains as a non-delivering `410 Gone` compatibility response that directs clients to `/settings/notifications`; the canonical account-channel test endpoint remains unchanged.
