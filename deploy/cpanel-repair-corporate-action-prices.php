<?php
/**
 * Detect and repair OHLCV rows not restated after split/bonus corporate actions (no SSH).
 *
 ...
 * Optional:   &profile=123 &stock=45 &action=7 &force=1
 *
...
echo "=== Repair corporate-action OHLCV prices ===\n\n";
...
echo 'Force ambiguous: '.($force ? 'yes' : 'no')."\n\n";
...
try {
    ...
    if (! $apply) {
        ...
        echo "\n";
        if ($needsRepair > 0) {
            echo "{$needsRepair} action(s) may need repair.\n";
